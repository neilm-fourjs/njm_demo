...
-- Create the database.
--
-- $Id: mkdb_mysql.4gl 718 2011-09-26 11:41:08Z neilm $
...
CONSTANT DEF_DB_DIR="/opt/fourjs/mysql_data"
...
FUNCTION mkdb(db,un,pw)
	...
	LET cmd = 'echo "create database ',db.trim(),';" > mkdb.sql'
	DISPLAY "mkdb: Execute:",cmd
	RUN cmd

	LET cmd = "mysql "
	IF un IS NOT NULL AND un.getLength() > 2 THEN
		LET cmd = cmd.append(" --user="||un)
	END IF
	IF pw IS NOT NULL AND pw.getLength() > 2 THEN
		LET cmd = cmd.append(" --password="||pw)
	END IF
	LET cmd = cmd.append(" < mkdb.sql")
	DISPLAY "mkdb: Execute:",cmd
	RUN cmd
